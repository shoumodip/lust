...
This document will also refer to Javascript in order to explaing the
...
Notice the conversion from interger to float. All numbers are floating
...
** Wrong concept
Let's try to imagine this in the Javascript POV from our previous
...
if user.madeFunOfLisp() {
    ...
    console.log("You did the right decision")
    console.log("Use JS! Everything else sucks!")
} else {
    console.log("LISP: Lots of Irritation Superfluous Parenthesis")
}
#+end_src
...
the phrase "sligtly intelligent" to describe Einstein. While not
...
* Macro example
In case you don't remember from earlier, here is an example of it.
...
Before explainations, let's run it.
...
#+begin_src lisp
> `(1 2 3 ,((+ 2 3 4) (+ 2 3 4)))
> `(1 2 3 (9 9))                  ; Evaluates the list before unwraping it
error: invalid function '9'
#+end_src

In Javascript lingo, "undefined is not a function".

This is where the ~,@body~ thingy comes into play. Instead of the
standard ~unquote~ behaviour, it unquotes expressions variadically.

#+begin_src lisp
> `(1 2 3 ,@((+ 2 3 4) (+ 2 3 4)))
> `(1 2 3 ,(+ 2 3 4) ,(+ 2 3 4))  ; Unwraps the list before evaluation
> `(1 2 3 9 9)                    ; => (1 2 3 9 9)
#+end_src

Note that ~,@EXPR~ is shorthand for ~(unquote-variadic EXPR)~
