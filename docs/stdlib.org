...
** ~(nth N SEQUENCE)~
Return the ~N~-th element of ~SEQUENCE~.
...
(defun sub (first :rest numbers)
  (let ((result first))
    (dolist (i numbers)
      (-= result i))
    result))

(function->list add)  ; => (lambda (:rest numbres) (let ((result 0)) (dolist (i numbers) (+= result i)) result))
(function->list sub)  ; => (lambda (first :rest numbres) (let ((result first)) (dolist (i numbers) (+= result i)) result))

(defmacro echo (:rest list)
  (dolist (i list)
    (print i)))

(defmacro echo (first :rest list)
  (print first)
  (dolist (i list)
    (print i)))
...
Set the ~N~-th element of ~LIST~ to ~VALUE~.
...
(set-nth list 1 'meh)
(print list)            ; => (1 'meh 3 4 5)

(set-nth '(1 2) 1 'lol) ; => (1 lol)
#+end_src
** ~(ns NAME [BINDINGS])~
