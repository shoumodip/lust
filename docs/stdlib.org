...
Set the ~N~-th element of ~LIST~ to ~VALUE~. The list supplied must be
defined in the current scope, anonymous lists are not allowed.
...
(set-nth '(1 2) 1 'lol) ; => error: invalid binding '(quote (1 2))' in set
#+end_src
